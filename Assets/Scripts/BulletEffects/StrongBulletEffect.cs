...
public class StrongBulletEffect : MonoBehaviour, IBulletEffect
{
	[SerializeField]
	float force;
	...
	void Start()
	{
		...
		if(!mesh)
		{
			Debug.LogError("StrongBulletEffect: Unable to find the mesh renderer!");
		}
	}

	public void Fire(Vector3 firingLocation, Vector3 direction, Vector3 fxLocation)
	{
		Debug.Log("StrongBulletEffect: Firing!");
	...
	{
		Debug.Log("StrongBulletEffect: I'm hitting something!");
		IDamageComponent target = other.gameObject.GetComponent<IDamageComponent>();
		if (target != null)
		{
			Debug.Log("StrongBulletEffect: It was the IDamageComponent of " + other.name);
			OnHit(target);
		}
	}
